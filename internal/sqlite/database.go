...
func (db *Database) CreateTable(ctx *sql.Context, name string, schema sql.Schema) (finalErr error) {
	var (
		defs []string
		pks  []string
	)

	txn, err := db.w.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	defer func() {
	...
	if _, err := txn.Exec(fmt.Sprintf(`CREATE TABLE "%s" (%s)`, name, strings.Join(defs, ", "))); err != nil {
		return err
	}
	return nil
}
